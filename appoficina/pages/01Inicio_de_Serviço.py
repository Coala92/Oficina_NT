...
nome_mecanico = list(df_ofi_inicio['Mecanico'].unique())
nome_mecanico_selecionado = st.sidebar.selectbox('Selecionar Mecânico:', nome_mecanico)

tipo_de_servico = list(df_ofi_inicio['Tipo de Servico'].unique())
tipo_de_servico_selecionado = st.sidebar.selectbox('Selecionar Tipo de Serviço:', tipo_de_servico)

tempo_estimado = list(df_ofi_inicio['Tempo Estimado'].unique())
tempo_estimado_selecionado = st.sidebar.selectbox('Selecionar Tempo Estimado:', tempo_estimado)


iniciar_servico = st.sidebar.button('Iniciar Serviço')
if iniciar_servico:
    lista_adicionar = [df_ofi_inicio['N° Serviço'].max() +1,
                       nome_mecanico_selecionado,
                       tipo_de_servico_selecionado,
                       tempo_estimado_selecionado,
                        ]
    hora_adicionar = datetime.now()
    df_ofi_inicio.loc[hora_adicionar] = lista_adicionar
    caminho_datasets = st.session_state['caminho_datasets']
    df_ofi_inicio.to_csv(caminho_datasets / 'df_ofi_inicio.csv', decimal=',', sep=';')

st.dataframe(df_ofi_inicio, hide_index=False, height=800)